...
function Badge({
  ...
  return (
    <span
      className={`inline-flex rounded-full border px-3 py-1.5 text-[12px] font-bold ${
        statusClasses[
          status
        ] ||
        statusClasses.New
...
const sourceClasses = {
  "Contact Page": "border-blue-200 bg-blue-50 text-blue-700",
  "Get In Touch Popup": "border-[#D9CBF5] bg-[#F3EEFB] text-[#6030C6]",
  "Footer CTA Popup": "border-orange-200 bg-orange-50 text-orange-700",
  "Product Page Popup": "border-emerald-200 bg-emerald-50 text-emerald-700",
  Website: "border-gray-200 bg-gray-50 text-gray-600",
};

function SourceBadge({ source }) {
  const label = source || "Website";
  return (
    <span className={`inline-flex rounded-full border px-3 py-1.5 text-[12px] font-bold ${sourceClasses[label] || sourceClasses.Website}`}>
      {label}
    </span>
  );
}
...
function DetailRow({
  ...
  return (
    <div className="min-h-[104px] rounded-[20px] border border-[#EEE6FB] bg-[#FBF8FF] p-5 shadow-[0_10px_24px_rgba(49,29,91,0.035)]">
      ...
      <div className="mt-3 break-words text-[15px] leading-7 text-[#2A2238]">
        {children}
      </div>
    </div>
  );
...
export default function AdminInquiriesPage() {
  ...
  const canView = isSuperAdmin(user) || hasPermission(user, "inquiries", "view");
  const canEdit = isSuperAdmin(user) || hasPermission(user, "inquiries", "edit");
  const canDelete = isSuperAdmin(user) || hasPermission(user, "inquiries", "delete");
    ...
    <div className="mx-auto w-full max-w-[1600px]">
                        ...
                        >
                          ...
                          <td className="px-5 py-4">
                            <div className="flex justify-end gap-2">
                              ...
                              <button
                                type="button"
                                onClick={() => setDeleteTarget(inquiry)} disabled={!canDelete}
                                className="inline-flex items-center gap-2 rounded-lg bg-red-50 px-3 py-2 text-[13px] font-bold text-red-600"
                              >
              ...
              {inquiries.map(
                (
                  ...
                  <article
                    ...
                    className="rounded-[18px] border border-[#E8E1F3] bg-white p-5"
                  >
      ...
      {selectedInquiry ? (
        <div className="fixed inset-0 z-[220] flex items-center justify-center bg-[#17112D]/55 p-3 backdrop-blur-[3px] sm:p-6">
          <div className="flex max-h-[92vh] w-full max-w-[960px] flex-col overflow-hidden rounded-[28px] bg-white shadow-2xl">
            <div className="flex items-start justify-between bg-[linear-gradient(135deg,#6030C6,#6E3DE2,#FF8626)] px-7 py-7 text-white">
              <div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-6 sm:p-8">
              ...
              <div className="mt-5 grid gap-5 md:grid-cols-2">
                <DetailRow icon={MessageSquare} label="Source">
                  <SourceBadge source={selectedInquiry.source} />
                </DetailRow>

                <DetailRow icon={MapPin} label="Location">
                  {selectedInquiry.location || "Not provided"}
                </DetailRow>
              </div>

              <div className="mt-5 min-h-[128px] rounded-[22px] border border-[#EEE6FB] bg-[#FBF8FF] p-6 shadow-[0_10px_24px_rgba(49,29,91,0.035)]">
                <p className="text-[12px] font-bold uppercase tracking-[0.1em] text-[#8B83A0]">
                  Message
                </p>
              ...
              <div className="mt-5 rounded-[22px] border border-[#DDD0F4] bg-[#FBF8FF] p-6 shadow-[0_10px_24px_rgba(49,29,91,0.035)]">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-[12px] font-bold uppercase tracking-[0.1em] text-[#6030C6]">
                      Inquiry Status
                    </p>

                    <p className="mt-2 text-[14px] text-[#777]">
                      Update the current customer follow-up status.
                    </p>
                  </div>
                ...
                <div className="mt-6 flex items-center gap-3">
                  <select
                    ...
                    disabled={!canEdit || savingId === selectedInquiry._id}
                    className="h-[48px] flex-1 rounded-xl border border-[#DDD7E8] bg-white px-4 text-[14px] font-bold outline-none focus:border-[#6030C6]"
                  >
                    {STATUSES.map(
                      (
                        status
            ...
            <div className="flex justify-between border-t border-[#EEE6FB] bg-white px-6 py-5">
              <button
                type="button"
                onClick={() => setDeleteTarget(selectedInquiry)} disabled={!canDelete}
                className="inline-flex h-12 items-center gap-2 rounded-xl border border-red-200 px-5 text-[14px] font-bold text-red-600 transition hover:bg-red-50"
              >
                <Trash2 className="h-4 w-4" />
                Delete
              </button>
              ...
              <button
                ...
                className="h-12 rounded-xl border border-[#DDD7E8] px-6 text-[14px] font-bold text-[#666] transition hover:bg-[#FBF8FF]"
              >
                Close
              </button>
            </div>
